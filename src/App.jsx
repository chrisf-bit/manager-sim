...
export default function App() {
  ...
  const radarSource = profileQuarter === 'current' ? metrics : (history[profileQuarter] || metrics);
  const radar = [{ metric: 'Trust', value: radarSource.trust }, { metric: 'Engagement', value: radarSource.engagement }, { metric: 'Performance', value: radarSource.performance }, { metric: 'Retention', value: radarSource.retention }, { metric: 'Fairness', value: radarSource.fairness }, { metric: 'Credibility', value: radarSource.credibility }];
      ...
      <div id="game-content" className="game-content" style={{ flex: 1, display: 'flex', overflow: 'hidden', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        {tab === 'insights' && (
          <div role="tabpanel" id="insights-panel" aria-label="Insights view" className="insights-grid" style={{ flex: 1 }}>
            ...
            <section aria-labelledby="profile-chart-heading" style={{ background: `linear-gradient(135deg, ${COLORS.grey} 0%, ${COLORS.greyDark} 100%)`, borderRadius: 16, padding: 20, border: `1px solid ${COLORS.white}10` }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}><h2 id="profile-chart-heading" style={{ fontSize: '0.75rem', color: COLORS.teal, textTransform: 'uppercase', letterSpacing: 2, fontWeight: 600, margin: 0 }}>Profile{profileQuarter !== 'current' ? ` — Q${history[profileQuarter]?.round || profileQuarter}` : ''}</h2><div style={{ display: 'flex', gap: 4 }}>{history.filter(h => h.round > 0).map((h) => { const hIdx = history.indexOf(h); return (<button key={h.round} onClick={() => setProfileQuarter(hIdx)} aria-label={`View Q${h.round} profile`} style={{ padding: '3px 10px', fontSize: '0.7rem', fontWeight: 600, fontFamily: "'Poppins', sans-serif", borderRadius: 6, border: profileQuarter === hIdx ? `1px solid ${COLORS.teal}` : `1px solid ${COLORS.white}20`, background: profileQuarter === hIdx ? `${COLORS.teal}25` : 'transparent', color: profileQuarter === hIdx ? COLORS.teal : `${COLORS.white}90`, cursor: 'pointer', transition: 'all 0.2s ease' }}>Q{h.round}</button>); })}<button onClick={() => setProfileQuarter('current')} aria-label="View current profile" style={{ padding: '3px 10px', fontSize: '0.7rem', fontWeight: 600, fontFamily: "'Poppins', sans-serif", borderRadius: 6, border: profileQuarter === 'current' ? `1px solid ${COLORS.teal}` : `1px solid ${COLORS.white}20`, background: profileQuarter === 'current' ? `${COLORS.teal}25` : 'transparent', color: profileQuarter === 'current' ? COLORS.teal : `${COLORS.white}90`, cursor: 'pointer', transition: 'all 0.2s ease' }}>Now</button></div></div><ResponsiveContainer width="100%" height="85%"><RadarChart data={radar} aria-label="Radar chart showing your leadership profile across all metrics"><PolarGrid stroke={`${COLORS.white}20`} /><PolarAngleAxis dataKey="metric" tick={{ fill: `${COLORS.white}99`, fontSize: 10 }} /><PolarRadiusAxis domain={[0, 100]} tick={{ fill: `${COLORS.white}70`, fontSize: 9 }} axisLine={false} /><Radar dataKey="value" stroke={profileQuarter === 'current' ? COLORS.purple : COLORS.teal} fill={profileQuarter === 'current' ? COLORS.purple : COLORS.teal} fillOpacity={0.3} strokeWidth={2} name="Score" /></RadarChart></ResponsiveContainer></section>
